const translations = {
  en: {
    ...
    "home.empty.description": "Create your first patient to start uploading and viewing OCT scans.",
    ...
    "home.dialog.new.description": "Enter a name and select the eye for this patient.",
    ...
    "home.dialog.delete.warning": "? This will permanently remove the patient and all associated images. This action cannot be undone.",
    ...
    "guide.description": "Learn how to use the 3D OCT Viewer for segmentation visualization and quantitative analysis.",
    "guide.s1.title": "1. Overview",
    "guide.s1.content": "3D OCT Viewer is a web-based analysis tool for visualizing and quantifying OCT (Optical Coherence Tomography) segmentation masks in interactive 3D. It computes ETDRS-based volume measurements and average layer thicknesses in real physical scale, and renders 3D meshes of retinal layers and fluid accumulations. All processing runs entirely in your browser — no server or data upload is required.",
    "guide.s2.title": "2. Getting Started",
    "guide.s2.step1.label": "Create a Patient",
    "guide.s2.step1.text": "Click \"New Patient\" on the home page. Enter a name and select the eye laterality (OD or OS).",
    "guide.s2.step2.label": "Upload OCT Images",
    "guide.s2.step2.text": "Open the patient page, switch to the \"OCT Images\" tab, and drag-and-drop your OCT images (PNG or JPG). File names should follow a natural order (e.g., p1.png through p25.png).",
    "guide.s2.step3.label": "Upload Segmentation Masks",
    "guide.s2.step3.text": "Switch to the \"Masks\" tab and upload the corresponding segmentation mask PNGs. Each pixel's R-channel value represents a label ID (0 = background, 1–N = tissue classes). Filenames must match the OCT images for correct pairing.",
    "guide.s2.step4.label": "Configure Labels",
    "guide.s2.step4.text": "After uploading masks, detected labels appear in the sidebar. Rename them (e.g., Layer 1 → NSR, Layer 3 → SRF) and assign colors by clicking the color swatch.",
    "guide.s3.title": "3. Image Viewer",
    "guide.s3.content": "Click any OCT image thumbnail to open the full-screen viewer. When both images and masks are uploaded for the same eye, colored edges of each segmentation label are automatically overlaid on the OCT images.",
    "guide.s3.f1": "Zoom in/out with scroll wheel or the on-screen controls, pan by clicking and dragging",
    "guide.s3.f2": "Navigate between images with arrow keys or the bottom controls",
    "guide.s3.f3": "Toggle edge overlay with the Layers button (bottom-left)",
    "guide.s3.f4": "Adjust edge thickness with the slider in the legend panel (bottom-right)",
    "guide.s4.title": "4. 3D Visualization",
    "guide.s4.content": "Click \"View in 3D\" on the patient page to generate interactive 3D meshes from the segmentation masks. The pipeline uses marching cubes with Z-axis interpolation, anisotropic Gaussian blur, and Taubin smoothing to produce smooth, volume-preserving surfaces.",
    ...
    "guide.s4.f5": "Cross-section slider to clip the volume along the depth axis",
    "guide.s4.f6": "Slices panel: toggle the slice grid to see where each uploaded image corresponds to in the 3D render, show or hide individual slices to inspect specific cross-sections within the volume",
    "guide.s5.title": "5. Measurements",
    "guide.s5.content": "All measurements are computed in real physical scale using the default OCT spacings (X: 11.54 µm, Y: 3.87 µm, Z: 246.0 µm).",
    "guide.s5.f1": "Average thickness (µm): mean layer thickness across all D×W columns",
    "guide.s5.f2": "Total volume (nL): overall volume per label",
    "guide.s5.f3": "ETDRS volumes (nL): per-region volumes displayed on a 9-sector circular grid",
    "guide.s5.f4": "ETDRS regions: center (1 mm), inner ring (3 mm), outer ring (6 mm)",
    "guide.s5.f5": "Nasal/Temporal orientation adapts to the selected eye (OD vs OS)",
    "guide.s5.f6": "Export all measurements to CSV via the sidebar export button",
    "guide.s6.title": "6. Input Format",
    "guide.s6.content": "Masks must be grayscale PNGs where each pixel value encodes a class label (R channel). A typical set-up:",
    ...
    "guide.s7.content": "All data (images, masks, measurements) is stored locally in your browser's IndexedDB. Nothing is uploaded to any server. Clearing your browser data will remove all stored patients and images.",
  },
  ...
  tr: {
    ...
    "home.empty.description": "OCT taramalarını yüklemeye ve görüntülemeye başlamak için ilk hastanızı oluşturun.",
    ...
    "home.dialog.new.description": "Bu hasta için bir isim girin ve göz tarafını seçin.",
    ...
    "home.dialog.delete.description": "Şunu silmek istediğinizden emin misiniz:",
    "home.dialog.delete.warning": "? Bu işlem hastayı ve ilişkili tüm görüntüleri kalıcı olarak kaldıracaktır. Bu işlem geri alınamaz.",
    ...
    "patient.uploadMasksFirst": "3B görünümü etkinleştirmek için maske yükleyin",
    ...
    "upload.dragDropOct": "OCT görüntülerini sürükleyip bırakın veya tıklayarak seçin",
    "upload.dragDropMasks": "Maske görüntülerini sürükleyip bırakın veya tıklayarak seçin",
    ...
    "guide.description": "Segmentasyon görselleştirme ve nicel analiz için 3B OCT Görüntüleyici'yi nasıl kullanacağınızı öğrenin.",
    "guide.s1.title": "1. Genel Bakış",
    "guide.s1.content": "3B OCT Görüntüleyici, OCT (Optik Koherens Tomografi) segmentasyon maskelerini etkileşimli 3B ortamda görselleştirmek ve nicelleştirmek için web tabanlı bir analiz aracıdır. ETDRS tabanlı hacim ölçümlerini ve ortalama tabaka kalınlıklarını gerçek fiziksel ölçekte hesaplar, retina tabakalarının ve sıvı birikimlerinin 3B modellerini oluşturur. Tüm işlemler tamamen tarayıcınızda çalışır — sunucu veya veri yüklemesi gerekmez.",
    "guide.s2.title": "2. Başlarken",
    "guide.s2.step1.label": "Hasta Oluşturma",
    "guide.s2.step1.text": "Ana sayfada \"Yeni Hasta\" butonuna tıklayın. Bir isim girin ve göz tarafını (OD veya OS) seçin.",
    "guide.s2.step2.label": "OCT Görüntüleri Yükleme",
    "guide.s2.step2.text": "Hasta sayfasını açın, \"OCT Görüntüleri\" sekmesine geçin ve OCT görüntülerinizi (PNG veya JPG) sürükleyip bırakın. Dosya adları doğal bir sıra izlemelidir (ör. p1.png'den p25.png'ye).",
    "guide.s2.step3.label": "Segmentasyon Maskeleri Yükleme",
    "guide.s2.step3.text": "\"Maskeler\" sekmesine geçin ve ilgili segmentasyon maskesi PNG'lerini yükleyin. Her pikselin R kanalı değeri bir etiket kimliğini temsil eder (0 = arka plan, 1–N = doku sınıfları). Doğru eşleştirme için dosya adları OCT görüntüleriyle eşleşmelidir.",
    "guide.s2.step4.label": "Etiketleri Yapılandırma",
    "guide.s2.step4.text": "Maskeleri yükledikten sonra, tespit edilen etiketler kenar çubuğunda görünür. Bunları yeniden adlandırın (ör. Katman 1 → NSR, Katman 3 → SRF) ve renk örneğine tıklayarak renk atayın.",
    "guide.s3.title": "3. Görüntü İzleyici",
    "guide.s3.content": "Tam ekran görüntüleyiciyi açmak için herhangi bir OCT görüntü küçük resmine tıklayın. Aynı göz için hem görüntüler hem de maskeler yüklendiğinde, her segmentasyon etiketinin renkli kenarları otomatik olarak OCT görüntülerinin üzerine bindirilir.",
    "guide.s3.f1": "Kaydırma tekerleği veya ekran kontrolleriyle yakınlaştırma/uzaklaştırma, tıklayıp sürükleyerek kaydırma",
    "guide.s3.f2": "Ok tuşları veya alt kontrollerle görüntüler arasında gezinme",
    "guide.s3.f3": "Katmanlar butonu (sol alt) ile kenar bindirmesini açma/kapama",
    "guide.s3.f4": "Gösterge panelindeki (sağ alt) kaydırıcı ile kenar kalınlığını ayarlama",
    ...
    "guide.s4.f4": "Katman bazında görünürlük açma/kapama ve opaklık kaydırıcıları",
    "guide.s4.f5": "Hacmi derinlik ekseni boyunca kesmek için kesit kaydırıcısı",
    "guide.s4.f6": "Kesitler paneli: her yüklenen görüntünün 3B render'da nereye karşılık geldiğini görmek için kesit ızgarasını açıp kapatma, hacim içindeki belirli kesitleri incelemek için bireysel kesitleri gösterme veya gizleme",
    "guide.s5.title": "5. Ölçümler",
    "guide.s5.content": "Tüm ölçümler varsayılan OCT aralıkları (X: 11,54 µm, Y: 3,87 µm, Z: 246,0 µm) kullanılarak gerçek fiziksel ölçekte hesaplanır.",
    "guide.s5.f1": "Ortalama kalınlık (µm): tüm D×W sütunlarındaki ortalama tabaka kalınlığı",
    "guide.s5.f2": "Toplam hacim (nL): etiket başına toplam hacim",
    "guide.s5.f3": "ETDRS hacimleri (nL): 9 sektörlü dairesel ızgarada bölge başına hacimler",
    "guide.s5.f4": "ETDRS bölgeleri: merkez (1 mm), iç halka (3 mm), dış halka (6 mm)",
    "guide.s5.f5": "Nazal/Temporal yönlendirme seçilen göze göre uyarlanır (OD ve OS)",
    "guide.s5.f6": "Kenar çubuğundaki dışa aktarma butonu ile tüm ölçümleri CSV'ye aktarma",
    "guide.s6.title": "6. Girdi Formatı",
    "guide.s6.content": "Maskeler, her piksel değerinin bir sınıf etiketini kodladığı (R kanalı) gri tonlamalı PNG'ler olmalıdır. Tipik bir yapılandırma:",
    ...
    "guide.s7.content": "Tüm veriler (görüntüler, maskeler, ölçümler) tarayıcınızın IndexedDB'sinde yerel olarak depolanır. Hiçbir şey herhangi bir sunucuya yüklenmez. Tarayıcı verilerinizi temizlemek, tüm kayıtlı hastaları ve görüntüleri kaldıracaktır.",
  },
} as const;
